      ...
      <View style={styles.reviewItemTextContainer}>
        ...
        <View>
          ...
          <View style={styles.reviewItemLikeDislike}>
            <TouchableOpacity
              onPress={() => props.likeDislike(props.review.id, "like")}
            >
              <Text>Like ({Array.isArray(likes) ? likes.length : 0})</Text>
            </TouchableOpacity>
            <TouchableOpacity
              onPress={() => props.likeDislike(props.review.id, "dislike")}
            >
              <Text>
                Dislike ({Array.isArray(dislikes) ? dislikes.length : 0})
              </Text>
            </TouchableOpacity>
          </View>
        </View>
...
const Reviews = props => {
  ...
  const likeDislike = async (review_id, col = "like") => {
    ...
    calculatedReview =
      Array.isArray(product.rating_sum) && product.rating_sum.length > 0
        ? Math.round(product.rating_sum[0].rating * 10) / 10
        : 0;
  ...
  const reviews = shop.PRODUCT_REVIEWS_BY_ID
    ? shop.PRODUCT_REVIEWS_BY_ID.reviews
    : [];
  ...
  return (
    <View style={styles.container}>
      <View style={styles.titleContainer}>
        ...
        <TouchableOpacity
          onPress={() =>
            props.navigation.navigate("Reviews", { productId: product.id })
          }
        >
